...
<form action="addproduct.php" method="post"  style="border:1px solid #ccc; width: 75% padding: 20px; text-align: center;" enctype="multipart/form-data"  >
      <h2>ADD  PRODUCT</h2>  
      <hr>
    ...
    <input type="text"   name="price" required id="price" ><br>

    <label for="cat"><b>Choose Category:</b></label><br>
     <select class="form-control "  name="cat" style="width: 200px; margin-right: auto; margin:auto auto; ">
      <?php
      include'conn.php';
      $y=mysqli_query($con,"select * from category");
      while($y1=mysqli_fetch_array($y)) {
       echo'<option>'.$y1['name'].'</option>';
                      }
               ?>
         </select>
    ...
    <div class="clearfix">
      ...
      <button type="submit" class="addbtn" name="add" style="margin-left: 30px; ">Add Product</button>
    </div>
</form>
</div>
...
$category=isset($_POST['cat'])?$_POST['cat']:'';
  ...
  if (isset($_POST['add'])) {
   ...
   $r=mysqli_query($con,"insert into product(name,image,price,category) values('$name','$imgname','$price','$category')") ;
   if (isset($r)) 
   {
     echo'<script> alert ("add  suuccessfuly")</script>';}
     else{
     echo'
    <script>alert("add  faild");</script> ';}
  }
